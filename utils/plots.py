...
def cv2ImgAddText(img, text, left, top, textColor=(0, 255, 0), textSize=15):
    # 图像从OpenCV格式转换成PIL格式
    if (isinstance(img, np.ndarray)):  # 判断是否OpenCV图片类型
        img = Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
    draw = ImageDraw.Draw(img)
    fontText = ImageFont.truetype(str(Path(__file__).resolve().parents[1] / "Font" / "FiraMono-Medium.otf"), textSize)
...
def plot_one_box(x, img, color=None, label=None, ch_text=None, line_thickness=None):
    ...
    if label:
        ...
        cv2.rectangle(img, c1, c2, color, -1, cv2.LINE_AA)  # filled
        # cv2.putText(img, label, (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
        img_text = cv2ImgAddText(img, ch_text, c1[0], c2[1], (255, 255, 255), 30)  # todo: 文字以左上点坐标为准，即：c1[0], c2[1]
    return img_text
